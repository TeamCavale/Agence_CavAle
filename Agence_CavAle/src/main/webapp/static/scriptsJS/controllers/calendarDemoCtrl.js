cal.controller('CalendarDemoCtrl', function($scope, visiteProvider, $rootScope,
		$location) {
	'use strict';
			...
			if (callback != undefined && callback != "") {
				...
				$rootScope.visites.forEach(function(visite) {
					...
					var startTime = new Date(date.getFullYear(), date
							.getMonth(), date.getDate()+date.getDay(), date.getHours(), date
							.getMinutes());
					
					var endTime = new Date(date.getFullYear(), date.getMonth(),
							date.getDate()+date.getDay(), date.getHours() + 1, date
									.getMinutes());
